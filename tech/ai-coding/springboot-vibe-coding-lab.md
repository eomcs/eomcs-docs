...
```
Node.js와 Express를 사용하여 현재 폴더를 프론트엔드 프로젝트 폴더로 만들어줘.
- 프로젝트 이름: vibeapp-frontend
```

프론트엔드 웹 페이지를 구현하기
```
REST API 서버와 연동하는 웹 페이지를 만들려고 한다. 
너에게 어떤 정보를 알려줘야 하는지 프롬프트 예시를 작성해줘.
```

```
# REST API 서버에서 제공하는 OpenAPI 명세를 기반으로 웹 페이지를 생성해줘.

## 주요 요구사항:
- REST API 서버: http://localhost:8080
- REST API 서버가 제공하는 OpenAPI 명세: http://localhost:8080/v3/api-docs
- AJAX: Fetch API를 사용하여 REST API와 통신하도록 구현해줘.
- REST API 서버 인증 방식: 
  - Type: Bearer Token
  - Token: JWT 토큰
...
개발 환경과 운영환경에서의 REST API URL 분리하기
```
...
```
다음 프롬프트를 코딩 에이전트에게 작업시키기 좋게 작업 순서가 분해된 실행 프롬프트로 재작성해줘.

# Next.js 기반 프론트엔드 프로젝트를 구성해줘.
- Next.js 버전: 15.x 사용
- 프로젝트 이름: myapp-frontend-nextjs
- 프로젝트 디렉토리: 따로 프론트엔드 폴더를 만들지 않고, 현재 프로젝트 루트에서 Next.js 프로젝트를 생성해줘.
- 스택: TypeScript, App Router, Tailwind CSS 사용
- UI: shadcn/ui를 기본 컴포넌트 라이브러리로 사용하고 싶어.
- 기능: http://localhost:8080/v3/api-docs에서 제공하는 OpenAPI 명세를 기반으로 REST API와 연동하는 웹 페이지를 만들어줘.
- REST API 서버: http://localhost:8080
- REST API 서버 인증 방식: 
  - Type: Bearer Token
  - Token: JWT 토큰
  - Access Token + Refresh Token
  - 토큰 저장: JWT 토큰과 Refresh Token을 localStorage에 저장
- 상태 관리: 서버 상태 관리는 TanStack Query를 사용하고, 클라이언트 상태는 Zustand를 사용해줘.
- 레이아웃: 상단 내비게이션 바와 하단 푸터가 포함된 기본 레이아웃을 생성해줘."
```

### 1단계 - Next.js 프로젝트 생성
순수 Next.js + TS + Tailwind 기반 초기 스캐폴딩 생성
```
# 현재 디렉토리를 Next.js 프로젝트로 초기화해줘.
...
프로젝트가 생성되면 폴더 구조를 정리해서 보여줘.
```

### 2단계 - shadcn/ui 설정
UI 기본기 확보
```
현재 프로젝트에 shadcn/ui를 설정해줘.

요구사항:
- Tailwind 설정이 shadcn/ui와 호환되도록 구성
- 필요한 기본 설정 파일 생성 (components.json 등)
- 기본 UI 컴포넌트(Button, Card, Input, Dialog)를 추가
- globals.css 정리

설정 완료 후:
- 간단한 테스트 페이지에서 Button과 Card가 정상 렌더링되는지 확인해줘.
```

### 3단계 - 환경 변수 설정
개발/운영 환경 분리 준비
```
API 서버 주소를 환경 변수로 분리해줘.

요구사항:
- .env.local 파일 생성
- NEXT_PUBLIC_API_BASE_URL=http://localhost:8080
- 코드에서 직접 하드코딩하지 말고 환경 변수를 사용하도록 API 유틸 구조 설계
```

### 4단계 - OpenAPI 기반 API 구조 설계
Contract First 기반 API 계층 설계
```
http://localhost:8080/v3/api-docs 의 OpenAPI 명세를 기준으로
프론트엔드 API 구조를 설계해줘.

요구사항:
- src/lib/api 폴더 생성
- 공통 fetch 유틸 작성
- Authorization 헤더 자동 주입 구조 설계
- 타입 안전성을 고려한 구조 설계
- 엔드포인트별 모듈 분리 (auth.ts, post.ts 등)

우선 구조 설계만 하고, 실제 페이지 연결은 아직 하지 마.
```

### 5단계 - 인증 상태 관리 (Zustand)
인증 상태의 단일 진실 공급원(Single Source of Truth) 생성
```
Zustand를 설치하고 auth 상태 관리 스토어를 만들어줘.
...
토큰은 localStorage에 저장하도록 구현해줘.
초기 로드시 localStorage에서 토큰을 복원하도록 만들어줘.
```

### 6단계 - TanStack Query 설정
서버 상태 관리 준비
```
TanStack Query를 설치하고 전역 Provider를 설정해줘.

요구사항:
- app/layout.tsx에 QueryClientProvider 적용
- 기본 QueryClient 생성
- 에러 처리 전략 기본 구성
```

### 7단계 - 인증 API 연결 (Login + Refresh)
JWT + Refresh 기반 인증 흐름 완성
```
OpenAPI 명세를 참고하여 다음 기능을 구현해줘:

1) 로그인 API 호출
2) Access Token + Refresh Token 저장
3) 401 발생 시 Refresh API 자동 호출
4) Refresh 성공 시 재요청
5) Refresh 실패 시 로그아웃 처리

공통 fetch 유틸에 인터셉터 개념으로 구현해줘.
```
...
실제 REST 연동 완료
```
OpenAPI 명세에 정의된 API를 사용하여 웹 페이지를 구현해줘.

요구사항:
- TanStack Query 사용
- useQuery / useMutation 사용
- 성공 시 invalidateQueries 처리
- 로딩/에러 UI 구현
```
...
실서비스 구조 완성
```
인증이 필요한 페이지에 대해 보호 라우트를 구현해줘.

요구사항:
- 로그인하지 않은 경우 /login으로 리다이렉트
- 인증 여부는 Zustand auth store 기준
- App Router 기준으로 구현
```
